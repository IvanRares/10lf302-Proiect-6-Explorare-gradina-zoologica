...
void CreateTextures(const std::string& strExePath)
{
	...
	unsigned char* data = stbi_load((strExePath + "\\..\\Textures\\grass.jpg").c_str(), &width, &height, &nrChannels, 0);
	if (data) {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// load image, create texture and generate mipmaps
	data = stbi_load((strExePath + "\\..\\Textures\\Bricks.jpg").c_str(), &width, &height, &nrChannels, 0);
	if (data) {
...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Gradina Zoologica", NULL, NULL);
	if (window == NULL) {
